...
class Spider(Spider):
        ...
        if is_live:
            ...
            try:
                ...
                for f in video_streams:
                    codecs = (f.get('codecs') or '').lower()
                    mime = (f.get('mimeType') or '').lower()
                    if 'avc' in codecs or 'h264' in codecs:
                        h264_streams.append(f)
                    else:
                        other_streams.append(f)

                # 线路1: H264 (过滤低于480P)
                h264_urls = []
                if h264_streams:
                    # 按高度分组，每个高度取码率最高，过滤低于480P
                    h264_height_groups = {}
                    for f in h264_streams:
                        h = int(f.get('height', 0))
                        if h < 480:
                            continue
                        h264_height_groups.setdefault(h, []).append(f)
                    for h in sorted(h264_height_groups.keys(), reverse=True):
                        items = h264_height_groups[h]
                        best = max(items, key=lambda x: int(x.get('bitrate') or 0))
                        label = self._get_quality_label(h)
                        h264_urls.append(f'{label}${video_id}@{h}_h264')

                # 线路2: 其他格式(AV1, VP9) (过滤低于480P)
                other_urls = []
                if other_streams:
                    other_height_groups = {}
                    for f in other_streams:
                        h = int(f.get('height', 0))
                        if h < 480:
                            continue
                        other_height_groups.setdefault(h, []).append(f)
                    for h in sorted(other_height_groups.keys(), reverse=True):
                        items = other_height_groups[h]
                        best = max(items, key=lambda x: int(x.get('bitrate') or 0))
                        label = self._get_quality_label(h)
                        other_urls.append(f'{label}${video_id}@{h}_other')

                # 设置2条线路，线路1(H264)在前设为默认
                if h264_urls:
                    play_sources.append('H264')
                    play_urls.append('#'.join(h264_urls))
                if other_urls:
                    play_sources.append('AV1/VP9')
                    play_urls.append('#'.join(other_urls))
    ...
    def playerContent(self, flag, pid, vipFlags):
        ...
        if quality_or_type == 'live':
            ...
            # 点播：解析高度和编码类型
            # 新格式: {height}_h264 或 {height}_other
            if quality_or_type.endswith('_h264') or quality_or_type.endswith('_other'):
                parts = quality_or_type.rsplit('_', 1)
                if len(parts) == 2 and parts[1] in ('h264', 'other'):
                    height_str, codec_type = parts
                    if height_str.isdigit():
                        return self._play_video_by_height_and_type(video_id, int(height_str), codec_type)
            # 兼容旧格式（纯高度或 best 等）
            if quality_or_type.isdigit():
                return self._play_video_by_height_and_type(video_id, int(quality_or_type), 'h264')  # 默认 H264
            else:
                quality = quality_or_type if quality_or_type in ('best', '4k', '2k', '1080p') else 'best'
                return self._play_video(video_id, quality)

    # ---------- 按高度和 SDR/HDR 类型播放点播 ----------
    def _play_video_by_height_and_type(self, video_id, target_height, codec_type):
        try:
            data = self.yt_video.extract(video_id)
            formats = data.get('formats', [])
            video_streams = [f for f in formats if f.get('vcodec') != 'none' and f.get('acodec') == 'none']
            ...
            # 先过滤低于480P的流
            video_streams = [f for f in video_streams if int(f.get('height', 0)) >= 480]
            ...
            if codec_type == 'h264':
                ...
                for f in video_streams:
                    codecs = (f.get('codecs') or '').lower()
                    if 'avc' in codecs or 'h264' in codecs:
                        codec_streams.append(f)
            elif codec_type == 'other':
                codec_streams = []
                for f in video_streams:
                    codecs = (f.get('codecs') or '').lower()
                    if 'avc' not in codecs and 'h264' not in codecs:
                        codec_streams.append(f)
            else:
                codec_streams = video_streams

            if not codec_streams:
                # 如果过滤后没有流，尝试不过滤编码但保持480P限制
                codec_streams = [f for f in formats if f.get('vcodec') != 'none' and f.get('acodec') == 'none' and int(f.get('height', 0)) >= 480]

            # 按高度精确匹配：先找相同高度的，找不到找最接近的（优先不高于目标）
            exact_matches = [f for f in codec_streams if int(f.get('height', 0)) == target_height]
            if exact_matches:
                candidates = exact_matches
            else:
                # 找最接近目标高度的（优先不超过目标的最高者，或最低的超目标者）
                lower = [f for f in codec_streams if int(f.get('height', 0)) < target_height]
                higher = [f for f in codec_streams if int(f.get('height', 0)) > target_height]
                if lower:
                    # 不超过目标的最大高度
                    max_lower_h = max(int(f.get('height', 0)) for f in lower)
                    candidates = [f for f in lower if int(f.get('height', 0)) == max_lower_h]
                elif higher:
                    # 超过目标的最小高度
                    min_higher_h = min(int(f.get('height', 0)) for f in higher)
                    candidates = [f for f in higher if int(f.get('height', 0)) == min_higher_h]
                else:
                    candidates = codec_streams

            # 选码率最高的
            selected_video = max(candidates, key=lambda x: int(x.get('bitrate') or 0))
            audio = self.yt_video.choose_audio(formats)
            if audio:
                cache_key = f'yt_{video_id}_{target_height}_{codec_type}'
                ...
                return {
                    'parse': 0,
                    'jx': 0,
                    'url': f'http://127.0.0.1:9978/proxy?do=py&type=mpd&vid={video_id}&quality={target_height}_{codec_type}',
            ...
            debug_log('_play_video_by_height_and_type error', {'video_id': video_id, 'height': target_height, 'type': codec_type, 'error': repr(e)})
